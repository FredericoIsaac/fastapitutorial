...
def test_create_job(client, normal_user_token_headers):
    ...
    response = client.post("/jobs/create-job/", content=json.dumps(data), headers=normal_user_token_headers)
...
def test_read_all_jobs(client, normal_user_token_headers):
    ...
    client.post("/jobs/create-job/", content=json.dumps(data), headers=normal_user_token_headers)
    client.post("/jobs/create-job/", content=json.dumps(data), headers=normal_user_token_headers)
...
def test_update_a_job(client, normal_user_token_headers):
    ...
    response = client.put("/jobs/update/1", content=json.dumps(data), headers=normal_user_token_headers)
